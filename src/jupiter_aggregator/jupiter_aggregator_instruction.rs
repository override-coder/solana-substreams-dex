...
pub fn parse_instruction(program: &String,bytes_stream: Vec<u8>, account_indices: &Vec<u8>, input_accounts: &Vec<String>) -> Option<RouterInstruction> {
...
pub fn parse_inner_instruction(program: &String,bytes_stream: Vec<u8>, account_indices: &Vec<u8>, input_accounts: &Vec<String>) -> Option<InstructionSwapEvent> {
    if program != JUPITER_AGGREGATOR_V6_PROGRAM_ADDRESS {
        return None
    }
    let accounts = prepare_input_accounts(account_indices, input_accounts);
    let account = accounts.get(0).unwrap_or(&"".to_string()).to_string();
