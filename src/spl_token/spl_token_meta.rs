...
use crate::spl_token::spl_token_meta_instruction::{
    prepare_arg, prepare_input_accounts, INSTRUCTION_TYPE_CREATE, INSTRUCTION_TYPE_CREATE_METADATA_ACCOUNT,
    INSTRUCTION_TYPE_CREATE_METADATA_ACCOUNT_V2, INSTRUCTION_TYPE_CREATE_METADATA_ACCOUNT_V3,
};
    ...
    for trx in block.transactions_owned() {
        ...
        if let Some(transaction) = trx.transaction {
            ...
            for (idx, inst) in msg.instructions.into_iter().enumerate() {
                ...
                if let Some(parsed_arg_data) = get_arg(program, &inst.data) {
                    ...
                    .for_each(|inner_instruction| {
                        inner_instruction
                            ...
                            .for_each(|(inner_idx, inner_inst)| {
                                ...
                                if let Some(parsed_arg_data) = get_arg(program, &inner_inst.data) {
                                    process_token_metadata(
                                        &mut data,
                                        parsed_arg_data,
                                        &inner_inst.accounts,
...
fn process_token_metadata(
    ...
    if let Some(args) = &token_metadata_meta.args {
        ...
        token_metadata_meta.input_accounts =
            prepare_input_accounts(args.instruction_type.clone(), instruction_accounts, accounts);
    }
...
fn get_arg(program: &str, instruction_data: &Vec<u8>) -> Option<Arg> {
    if program != constants::TOKEN_METADATA_PROGRAM_ADDRESS {
        return None;
    }
    Some(prepare_arg(instruction_data))
}

fn filter_metadata_is_none(mut obj: &TokenMetadataMeta) -> bool {
    return obj.instruction_type != INSTRUCTION_TYPE_CREATE_METADATA_ACCOUNT
        && obj.instruction_type != INSTRUCTION_TYPE_CREATE_METADATA_ACCOUNT_V2
        && obj.instruction_type != INSTRUCTION_TYPE_CREATE_METADATA_ACCOUNT_V3
